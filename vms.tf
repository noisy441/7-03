...
resource "yandex_compute_instance" "web_c" {
  name        = "web-c" #Имя ВМ в облачной консоли
  hostname    = "web-c" #формирует FDQN имя хоста, без hostname будет сгенрировано случаное имя.
  platform_id = "standard-v3"
  zone        = "ru-central1-a" #зона ВМ должна совпадать с зоной subnet!!!
...
resource "local_file" "inventory" {
  ...
  [dbservers]
  ${yandex_compute_instance.web_c.network_interface.0.ip_address}
  [webservers:vars]
